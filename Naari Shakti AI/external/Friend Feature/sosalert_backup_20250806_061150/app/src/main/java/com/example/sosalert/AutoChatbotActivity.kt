        ...
        sendBtn.setOnClickListener {
            ...
            if (userInput.isNotEmpty() && !isProcessing) {
                ...
                lifecycleScope.launch {
                    try {
                        val result = aiService.sendMessage(userInput, AIChatbotService.AIProvider.OLLAMA)
                        addMessage("🤖 Legal Assistant: ${'$'}{result.content}")
                    } catch (e: Exception) {
                        // Fallback to simple local generator
                        val fallback = generateAutomaticResponse(userInput)
                        addMessage("🤖 Legal Assistant: ${'$'}fallback")
                    } finally {
                        isProcessing = false
                    }
                }
